...
void tag_cb(const apriltags_ros::AprilTagDetectionArray::ConstPtr& input)
{
  ...
  if ( tag_length == 4)
  {
    ...
    for(int i = 0; i < tag_length; i++)
    {
      ...
      else if (input->detections[i].id == 1)
      ...
      else if (input->detections[i].id == 2)
        {tag2.set(input->detections[i].pose.pose.position.x, input->detections[i].pose.pose.position.y, input->detections[i].pose.pose.position.z, r, p, y);}
      else if (input->detections[i].id == 3)
        {tag3.set(input->detections[i].pose.pose.position.x, input->detections[i].pose.pose.position.y, input->detections[i].pose.pose.position.z, r, p, y);}
    ...
    Pose3d_Node* tag2_node = new Pose3d_Node();
    slam.add_node(tag2_node);
    pose_nodes.push_back(tag2_node);
    Pose3d_Node* tag3_node = new Pose3d_Node();
    slam.add_node(tag3_node);
    pose_nodes.push_back(tag3_node);
    ...
    Pose3d_Factor* prior = new Pose3d_Factor(pose_nodes[4], origin, noise3);
    slam.add_factor(prior);  // add it to the graph
    //=================================================
    Pose3d_Pose3d_Factor* camera_tag0 = new Pose3d_Pose3d_Factor(pose_nodes[4], pose_nodes[0], tag0, noise2);
    slam.add_factor(camera_tag0);
    //=================================================
    Pose3d_Pose3d_Factor* camera_tag1 = new Pose3d_Pose3d_Factor(pose_nodes[4], pose_nodes[1], tag1, noise2);
    slam.add_factor(camera_tag1);
    //=================================================
    Pose3d_Pose3d_Factor* camera_tag2 = new Pose3d_Pose3d_Factor(pose_nodes[4], pose_nodes[2], tag2, noise2);
    slam.add_factor(camera_tag2);
    //=================================================
    Pose3d_Pose3d_Factor* camera_tag3 = new Pose3d_Pose3d_Factor(pose_nodes[4], pose_nodes[3], tag3, noise2);
    slam.add_factor(camera_tag3);
    //=================================================
    Pose3d fixed_0_1(0.0677, 0, 0, 0, 0, 0);
    Pose3d_Pose3d_Factor* fixed_0_1_factor = new Pose3d_Pose3d_Factor(pose_nodes[0], pose_nodes[1], fixed_0_1, noise3);
    slam.add_factor(fixed_0_1_factor);
    //=================================================
    Pose3d fixed_1_2(0.0677, 0, 0, 0, 0, 0);
    Pose3d_Pose3d_Factor* fixed_1_2_factor = new Pose3d_Pose3d_Factor(pose_nodes[1], pose_nodes[2], fixed_1_2, noise3);
    slam.add_factor(fixed_1_2_factor);
    //=================================================
    Pose3d fixed_2_3(0.0677, 0, 0, 0, 0, 0);
    Pose3d_Pose3d_Factor* fixed_2_3_factor = new Pose3d_Pose3d_Factor(pose_nodes[2], pose_nodes[3], fixed_2_3, noise3);
    slam.add_factor(fixed_2_3_factor);
    //=================================================
